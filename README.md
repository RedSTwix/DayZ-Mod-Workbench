...
O conversor Python v7 é tratado como addon do Workbench e fica em `addons\deodol_source_windows.py`, ao lado do executável. Antes do primeiro uso de cada execução, o programa consulta a [versão publicada no gist](https://gist.githubusercontent.com/RedSTwix/7dabd68cd538bc2d74b447829a1c7ea7/raw/deodol_source_windows.py), mas **a cópia local v7 é a base segura**. O arquivo remoto só pode substituir o local depois de passar a validação estrutural do addon v7 (incluindo recuperação forense RaP) e a comparação de versão; versões antigas nunca causam downgrade. Se o Gist ainda estiver em v6/legado, devolver conteúdo inválido ou a internet estiver indisponível, o Workbench simplesmente mantém a cópia local v7 válida. Quando o Gist for publicado com v7 compatível, o SHA-256 é comparado e a atualização é instalada de forma atômica. Instalações antigas em `P:\` ou `%LocalAppData%\DayZ Mod Workbench\tools` só são migradas se já passarem nos requisitos v7. A integração passa o `CfgConvert.exe` oficial por `--cfgconvert` e só aceita o PBO quando o relatório v7 retorna `SEMANTIC-EXACT` ou `SEMANTIC-EXACT-INCLUDE-GRAPH`, com todos os P3D e configs verificados.
...
O Workbench exige agora `PBO_TOOLS_MARKER_RECOVERY` e `PBO_FULL_PAYLOAD_RECOVERY` no addon, impedindo downgrade automático para uma revisão v7 que ainda não possua essas técnicas.
...
A capability `PBO_TOOLS_V18_MARKER_RECOVERY` passou a ser obrigatória no Workbench 1.7.6, impedindo downgrade automático para um addon v7 que só conheça o layout 1.9.x.
